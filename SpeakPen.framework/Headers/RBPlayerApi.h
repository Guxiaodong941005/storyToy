...
/**
 ...
 @param page 页数
 */
+ (void)getCollectionList:(NSInteger)type page:(NSInteger)page block:(void (^)(RBCollectionList *list ,NSError * _Nullable))block;

/**
 添加资源收藏
 返回结果字典数组 key 是当前资源rID val 是当前资源的fID 收藏ID
 */
+ (void)addCollection:(NSArray<RBCollectionModel*>*)collectionModels  block:(void (^)(NSArray<NSDictionary*>*ridArray, NSError* _Nullable error))block;
